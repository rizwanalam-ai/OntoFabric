...
export const extractOntologyFromText = async (rawText) => {
...
const asNeo4jProperties = (properties) => properties;
const withoutKeys = (properties, keys) => {
...
const parsePrimitiveDictionary = (value) => {
...
const parseProvenance = (value, properties) => {
    try {
        return z.object({
            sourceSystem: z.string(), rawSourceId: z.string(), filePath: z.string().optional(),
            lineNumber: z.number().int().optional(), extractionTimestamp: z.string(), rawPayload: z.string().optional(), mcpTool: z.string().optional()
...
const temporalDefaults = (validFrom, temporal) => ({
    ...
    try {
        await session.executeWrite(async (transaction) => {
            for (const node of nodes) {
                ...
                await transaction.run(`MATCH (previous:Entity {id: $id})
           WHERE previous.transactionTo > $transactionFrom
           SET previous.transactionTo = $transactionFrom`, { id: node.id, transactionFrom: temporal.transactionFrom });
                await transaction.run(`MERGE (n:Entity {id: $id, transactionFrom: $transactionFrom})
             SET n += $properties,
               n.id = $id,
               n.typeId = $typeId,
...
export const closeOntologyServices = async () => {
    if (neo4jDriver) {
