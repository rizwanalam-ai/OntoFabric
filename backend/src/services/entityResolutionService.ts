...
import { DEFAULT_TEMPORAL_END, type GraphNode, type PrimitiveDictionary } from '@ontofabric/shared/types.js';
import { z } from 'zod';
...
const jaroWinkler = (first: string, second: string): number => {
  ...
  for (let index = 0; index < aSequence.length; index += 1) {
    if (aSequence[index] !== bSequence[index]) transpositions += 1;
  }
...
const cosineSimilarity = (first: number[], second: number[]): number => {
...
export const calculateSimilarity = async (entityA: GraphNode, entityB: GraphNode): Promise<number> => {
  ...
  const propertyScore = propertyKeys.length === 0 ? nameScore : propertyKeys.reduce((sum, key) => (
    sum + (entityA.properties[key] !== undefined && entityA.properties[key] === entityB.properties[key] ? 1 : 0)
  ), 0) / propertyKeys.length;
...
const nodeFromRecord = (raw: { properties: Record<string, unknown> }): GraphNode => {
...
const pendingIdFor = (first: string, second: string): string => createHash('sha256')
  .update([first, second].sort().join('|'))
  .digest('hex')
  .slice(0, 24);

const mergeNodes = async (transaction: { run: (query: string, parameters?: Record<string, unknown>) => Promise<unknown> }, entityAId: string, entityBId: string, action: 'MERGE' | 'LINK'): Promise<void> => {
  await transaction.run(
    `MATCH (a:Entity {id: $entityAId}), (b:Entity {id: $entityBId})
...
export const findCandidateDuplicates = async (newNode: GraphNode, threshold = 0.70): Promise<CandidateDuplicate[]> => {
  const session = getNeo4jDriver().session();
  const candidates: CandidateDuplicate[] = [];
  try {
    const result = await session.executeRead((transaction) => transaction.run(
      'MATCH (n:Entity) WHERE n.typeLabel = $typeLabel AND n.id <> $id RETURN n LIMIT 500',
      { typeLabel: newNode.type.label, id: newNode.id }
    ));
    for (const record of result.records) {
      ...
      if (confidence >= 0.92) {
        ...
        await session.executeWrite((transaction) => transaction.run(
          ...
          { pendingId, entityAId: newNode.id, entityBId: existing.id, entityAJson: JSON.stringify(newNode), entityBJson: JSON.stringify(existing), confidence, conflictsJson: JSON.stringify(conflicts), createdAt: new Date().toISOString() }
        ));
  ...
  try {
    ...
    return result.records.map((record) => {
      ...
      return {
        ...
        entityA: JSON.parse(asText(properties.entityAJson)) as GraphNode,
        entityB: JSON.parse(asText(properties.entityBJson)) as GraphNode,
        confidence: Number(properties.confidence),
        conflicts: JSON.parse(asText(properties.conflictsJson)) as string[],
        status: 'PENDING',
...
export const resolvePendingMatch = async (pendingId: string, action: MatchAction): Promise<void> => {
  const session = getNeo4jDriver().session();
  try {
    await session.executeWrite(async (transaction) => {
      const result = await transaction.run(
        `MATCH (p:PendingReview {pendingId: $pendingId, status: 'PENDING'}) RETURN p`,
        { pendingId }
      );
      const pending = result.records[0]?.get('p')?.properties as Record<string, unknown> | undefined;
      if (!pending) throw new Error('Pending match was not found or was already resolved.');
      if (action === 'MERGE' || action === 'LINK') {
        await mergeNodes(transaction, asText(pending.entityAId), asText(pending.entityBId), action);
      }
      await transaction.run(
        `MATCH (p:PendingReview {pendingId: $pendingId})
         SET p.status = $status, p.resolvedAt = $resolvedAt, p.resolutionAction = $action`,
        { pendingId, status: action === 'REJECT' ? 'REJECTED' : 'RESOLVED', resolvedAt: new Date().toISOString(), action }
      );
    });
  } finally {
    await session.close();
