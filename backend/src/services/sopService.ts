...
import { DEFAULT_TEMPORAL_END, type GraphEdge, type GraphNode, type PrimitiveDictionary, type SopGraphPayload, type SopPlanningSummary, type SopRelationshipType } from '@ontofabric/shared/types.js';

import { getNeo4jDriver } from './ontologyService.js';

export const SOP_RELATIONSHIP_TYPES: readonly SopRelationshipType[] = [
  'HAS_DEMAND', 'FOR_PRODUCT', 'REQUIRES_BOM', 'STORED_AT', 'PRODUCED_AT', 'SUPPLIED_BY', 'SHIPPED_TO', 'FULFILLED_BY'
];

const primitiveProperties = (value: Record<string, unknown>): PrimitiveDictionary => Object.fromEntries(
  Object.entries(value).filter(([, item]) => typeof item === 'string' || typeof item === 'number' || typeof item === 'boolean' || item === null)
...
const resultNode = (node: { properties: Record<string, unknown> }): GraphNode => {
  ...
  return {
    ...
    provenance: (() => {
      try { return JSON.parse(String(properties.provenanceJson)); } catch { return { sourceSystem: 'SOP', rawSourceId: String(properties.id), extractionTimestamp: String(properties.createdAt ?? new Date().toISOString()) }; }
    })()
  };
};

export const querySopGraph = async (): Promise<{ nodes: GraphNode[]; edges: GraphEdge[] }> => {
...
export const querySopPlanningSummary = async (): Promise<SopPlanningSummary> => {
  const session = getNeo4jDriver().session();
  try {
...
export const getSopDriver = (): Driver => getNeo4jDriver();