...
router.post('/schema/drift-check', async (request, response) => {
  ...
  try {
    ...
    const result = await detectSchemaDrift(
      [parsedRequest.data.incomingSample],
      expectedSchema,
      parsedRequest.data.sourceSystem
    );
    response.json(result);
  } catch (error) {
    ...
    if (targetEntity) {
      try {
        ...
        const samples = Array.isArray(parsedSource)
          ? parsedSource.filter((item): item is Record<string, unknown> => Boolean(item) && typeof item === 'object' && !Array.isArray(item)).slice(0, 25)
        ...
        if (Array.isArray(parsedSource) && Object.keys(schemaDrift.healedMappings).length > 0) {
          sourceForExtraction = applyHealedMappings(parsedSource as Record<string, any>[], schemaDrift.healedMappings);
        }
      } catch {
...
router.post('/ingest/upload', upload.single('file'), async (request, response) => {
  ...
  try {
    ...
    if (parsedRequest.data.source === 'LOCAL') {
      if (!request.file) throw new Error('Select a PDF, Excel, CSV, or Word file first.');
    ...
    let sourceForExtraction: unknown = parsedFile.parsedSource;
    let schemaDrift: SchemaDriftResult | undefined;
    if (parsedRequest.data.targetEntity) {
      try {
        const expectedSchema = await resolveExpectedSchema(parsedRequest.data.targetEntity);
        const samples = parsedFile.sourceType === 'PDF' || parsedFile.sourceType === 'WORD'
          ? [parsedFile.parsedSource as Record<string, unknown>]
          : [parsedFile.parsedSource as Record<string, unknown>];
        ...
        if (Object.keys(schemaDrift.healedMappings).length > 0 && Array.isArray(sourceForExtraction)) {
          sourceForExtraction = applyHealedMappings(sourceForExtraction as Record<string, any>[], schemaDrift.healedMappings);
        }
      } catch {
