...
const typeStyles = [
  { accent: '#7dd3fc', tint: 'rgba(125, 211, 252, 0.16)' },
  { accent: '#c4b5fd', tint: 'rgba(196, 181, 253, 0.16)' },
  { accent: '#fda4af', tint: 'rgba(253, 164, 175, 0.16)' },
  { accent: '#fcd34d', tint: 'rgba(252, 211, 77, 0.16)' },
  { accent: '#86efac', tint: 'rgba(134, 239, 172, 0.16)' },
  { accent: '#67e8f9', tint: 'rgba(103, 232, 249, 0.16)' }
];

const getTypeStyle = (typeId: string) => {
  const hash = [...typeId].reduce((total, character) => total + character.charCodeAt(0), 0);
  return typeStyles[hash % typeStyles.length];
};
...
function OntologyNode({ data }: NodeProps<Node<ExplorerNodeData>>) {
  ...
  const typeStyle = getTypeStyle(graphNode?.type.id ?? members?.[0]?.type.id ?? 'cluster');
  const label = graphNode?.type.label ?? members?.[0]?.type.label ?? 'Group';

  return (
    <div className="min-w-[190px] max-w-[235px] rounded-2xl border bg-[#101a2c]/95 px-4 py-3 shadow-2xl shadow-black/30 transition-transform duration-200 hover:-translate-y-1" style={{ borderColor: data.highlighted ? '#f5f06a' : `${typeStyle.accent}99`, opacity: data.dimmed ? 0.25 : 1, boxShadow: data.highlighted ? '0 0 0 3px rgba(245, 240, 106, 0.24), 0 14px 38px rgba(0, 0, 0, 0.24)' : `0 14px 38px rgba(0, 0, 0, 0.24), 0 0 0 1px ${typeStyle.accent}20` }}>
      <Handle type="target" position={Position.Left} className="!h-2 !w-2 !border-2 !border-[#101a2c]" style={{ background: typeStyle.accent }} />
      <div className="mb-3 flex items-start justify-between gap-3"><span className="text-[10px] font-bold uppercase tracking-[0.18em]" style={{ color: typeStyle.accent }}>{label}</span><span className="rounded-full px-2 py-1 text-[9px] font-bold tracking-[0.12em]" style={{ color: sourceStyle.accent, backgroundColor: sourceStyle.tint }}>{members ? `${members.length} grouped` : sourceStyle.label}</span></div>
      ...
      <Handle type="source" position={Position.Right} className="!h-2 !w-2 !border-2 !border-[#101a2c]" style={{ background: typeStyle.accent }} />
    </div>
  );
}
