...
import { Expand, Fullscreen, GitBranch, Maximize2, Minimize2, Search, Waypoints } from 'lucide-react';
...
function OntologyNode({ data }: NodeProps<Node<ExplorerNodeData>>) {
  ...
  const style = sourceStyles[graphNode?.sourceSystem ?? members?.[0]?.sourceSystem ?? 'SME_INPUT'];
  const label = graphNode?.type.label ?? members?.[0]?.type.label ?? 'Group';

  return (
    <div className="min-w-[190px] max-w-[235px] rounded-2xl border bg-[#101a2c]/95 px-4 py-3 shadow-2xl shadow-black/30 transition-transform duration-200 hover:-translate-y-1" style={{ borderColor: data.highlighted ? '#f5f06a' : `${style.accent}99`, opacity: data.dimmed ? 0.25 : 1, boxShadow: data.highlighted ? '0 0 0 3px rgba(245, 240, 106, 0.24), 0 14px 38px rgba(0, 0, 0, 0.24)' : `0 14px 38px rgba(0, 0, 0, 0.24), 0 0 0 1px ${style.accent}20` }}>
      <Handle type="target" position={Position.Left} className="!h-2 !w-2 !border-2 !border-[#101a2c]" style={{ background: style.accent }} />
      <div className="mb-3 flex items-start justify-between gap-3"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">{label}</span><span className="rounded-full px-2 py-1 text-[9px] font-bold tracking-[0.12em]" style={{ color: style.accent, backgroundColor: style.tint }}>{members ? `${members.length} grouped` : style.label}</span></div>
      ...
      <Handle type="source" position={Position.Right} className="!h-2 !w-2 !border-2 !border-[#101a2c]" style={{ background: style.accent }} />
    </div>
  );
}
...
function RelationshipEdge({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, markerEnd, label }: EdgeProps) {
  ...
  return <><BaseEdge id={id} path={path} markerEnd={markerEnd} style={{ stroke: '#71829e', strokeWidth: 1.6 }} /><EdgeLabelRenderer><div className="nodrag nopan pointer-events-none absolute rounded-full border border-white/10 bg-[#0b1220]/95 px-2 py-1 font-mono text-[10px] font-semibold text-slate-300 shadow-lg" style={{ transform: `translate(-50%, -50%) translate(${labelX}px, ${labelY}px)` }}>{label}</div></EdgeLabelRenderer></>;
}
...
export function GraphExplorer({ nodes, edges, onNodeClick, timeline, highlightedNodeIds = [], onNodeContextMenu }: GraphExplorerProps) {
  ...
  return <section className={`${isFullscreen ? 'fixed inset-4 z-50 h-[calc(100vh-2rem)] w-[calc(100vw-2rem)]' : 'relative h-full min-h-[680px] min-w-0 flex-1'} overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#0a1221] shadow-2xl shadow-black/20`}>
    {isFullscreen && <div className="fixed inset-0 -z-10 bg-[#030711]/90 backdrop-blur-sm" />}
    <div className="absolute left-4 right-4 top-4 z-20 flex flex-wrap items-center gap-2 rounded-2xl border border-white/10 bg-[#101a2c]/95 p-2 shadow-xl shadow-black/20 backdrop-blur-md"><div className="flex items-center gap-2 px-2 text-[10px] font-bold uppercase tracking-[0.18em] text-cyan-300"><Waypoints size={14} /> Explorer</div><button type="button" onClick={() => setLayout('force')} className={`flex items-center gap-1 rounded-lg px-2.5 py-2 text-xs ${layout === 'force' ? 'bg-cyan-300 text-[#06111d]' : 'text-slate-400 hover:bg-white/10'}`}><GitBranch size={13} /> Force</button><button type="button" onClick={() => setLayout('hierarchical')} className={`flex items-center gap-1 rounded-lg px-2.5 py-2 text-xs ${layout === 'hierarchical' ? 'bg-cyan-300 text-[#06111d]' : 'text-slate-400 hover:bg-white/10'}`}><GitBranch size={13} /> Dagre</button><div className="relative min-w-[180px] flex-1 md:max-w-xs"><Search size={13} className="absolute left-3 top-2.5 text-slate-500" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Filter nodes" className="w-full rounded-lg border border-white/10 bg-[#0a1221] py-2 pl-8 pr-3 text-xs text-slate-200 outline-none placeholder:text-slate-600 focus:border-cyan-300/60" /></div><button type="button" onClick={() => void reactFlow?.fitView({ padding: 0.2, duration: 350 })} aria-label="Zoom to fit" title="Zoom to fit" className="rounded-lg p-2 text-slate-300 hover:bg-white/10 hover:text-white"><Maximize2 size={15} /></button><button type="button" onClick={() => setIsFullscreen((current) => !current)} aria-label={isFullscreen ? 'Exit fullscreen graph' : 'Open fullscreen graph'} title={isFullscreen ? 'Exit fullscreen graph' : 'Open fullscreen graph'} className="rounded-lg p-2 text-slate-300 hover:bg-white/10 hover:text-white">{isFullscreen ? <Minimize2 size={15} /> : <Fullscreen size={15} />}</button></div>
  ...
  {nodes.length === 0 ? <div className="flex h-full min-h-[680px] items-center justify-center text-sm text-slate-500">No S&amp;OP records found in Neo4j.</div> : <ReactFlow nodes={flowModel.nodes} edges={flowModel.edges} nodeTypes={nodeTypes} edgeTypes={edgeTypes} fitView fitViewOptions={{ padding: 0.25 }} onInit={setReactFlow} nodesDraggable nodesConnectable={false} panOnDrag zoomOnScroll onNodeClick={(_, node) => node.data.graphNode && onNodeClick(node.data.graphNode)} onNodeContextMenu={(event, node) => { event.preventDefault(); if (node.data.graphNode) setContextMenu({ node: node.data.graphNode, x: event.clientX, y: event.clientY }); }} onPaneClick={() => setContextMenu(null)} proOptions={{ hideAttribution: true }} className="bg-[radial-gradient(circle_at_50%_45%,rgba(21,54,82,0.26),transparent_48%)]"><Background color="#24354b" gap={28} size={1} /><Controls className="!bottom-5 !left-5 !z-30 !m-0 !overflow-hidden !rounded-xl !border-white/10 !bg-[#101a2c] !fill-slate-300" /><MiniMap nodeColor={(node) => sourceStyles[(node.data as ExplorerNodeData | undefined)?.graphNode?.sourceSystem ?? 'SOP']?.accent ?? '#8bd5ca'} maskColor="rgba(4, 9, 18, 0.75)" className="!bottom-5 !right-5 !m-0 !rounded-xl !border-white/10 !bg-[#101a2c]" /></ReactFlow>}
